...
class DBDepartmentSyncService(BaseSyncDBService):
    ...
    def created_handler(self):
        ...
        # 1. 使用 BFS 转换出可顺序插入的列表，使用 mptt 进行插入
        ...
        # 2. 以 mptt 方式添加部门，不可批量添加，因为存在依赖，添加时 parent 可能未存在
        ...
        for dept_id in sorted_departments:
            ...
            if dept.parent_id:
                try:
                    ...
                    # 记录错误信息，然后将异常往上抛，
                    # 因为 Django QuerySet get 异常并不会输出具体 ID 是什么，不利于问题排查
                    logger.exception(f"parent department(id:{dept.parent_id}) not found")
                    raise
            # 对于 mptt，必须是 parent 实例对象，无法使用 parent_id 代替
    ...
    def deleted_handler(self):
        ...
        # 1. 使用 BFS 转换出可顺序删除的列表，使用 mptt 进行删除
        ...
        # 2. 以 mptt 方式删除部门，不可批量删除，因为存在依赖，删除时可能前一个 parent 也在删除中，树无法变更
    ...
    def updated_parent_handler(self):
        ...
        # SaaS 使用 mptt 进行更新 parent
        for d in updated_parent_departments:
            # Note: 这里必须重新获取部门对象，否则会是旧的数据，这样会导致更新时 lft\rght\level 错误
    ...
    def updated_handler(self):
        """关于更新部门基本信息，DB 的处理"""
        # 只更新变更了的 name,category_id,order 的部门
...
class DBDepartmentSyncExactInfo(BaseSyncDBService):
    ...
    def _calculate_children(self, departments: List[Department]) -> Tuple[Set, Dict[int, List]]:
        """根据 id 和 parentID 的相邻关系，计算出每个节点的孩子列表和根节点"""
        root = set()  # 记录每棵树的根节点，后面用于遍历树
        # 存储每个节点的孩子
        children_map = defaultdict(list)
        for dept in departments:
            parent_id = dept.parent_id  # 这里不可使用 parent，否则由于 parent 是外键关联，会引起 DB 查询
            # 如果 parent 为 None，则为单独的树 root
            if not parent_id:
                root.add(dept.id)
                continue
            # 作为其他节点的孩子
    ...
    def _calculate_ancestors(
        ...
        """使用 BFS 从根节点遍历树的每个节点，计算出每个节点的祖先"""
        ancestors_map: Dict[int, List] = defaultdict(list)
        # 进行 BFS 遍历
        node_set = {i.id for i in departments}
        queue = list(root)  # 使用 index 来模拟队列即可，不使用 python 的 deque 数据结构
    ...
    def calculate_new_data(self) -> List[Dict]:
        ...
        2. 从 Root 遍历到叶子节点，当前节点祖先=直接上级的祖先 + 直接上级
        3. 查询 DepartmentMember，遍历每条记录，可得到每个部门的用户数 (不包含递归子部门的)
        4. 查询 DepartmentMember，
            遍历得到每个用户的所在的部门 (包括部门的祖先)，
            遍历每个用户，对于所在的每个部门人数 +1，得到每个部门的递归用户数
        """
        ...
        # NOTE: 以下计算是使用树等方式计算需要的数据，不可用直接查询 DB 计算，否则 DB 查询量将会非常大
